...
def make_database(
    ...
    p = subprocess.Popen(
        "makeblastdb -parse_seqids -in " + input_path + " -dbtype " + type + " -title " + name + " -out " + output_path,
...
def run_blast_align(
    ...
    p = subprocess.Popen(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, env=BLAST_ENV)
    p.wait()
